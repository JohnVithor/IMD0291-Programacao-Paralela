...
for args_tuple in itertools.product(*args["args"].values()):
  ...
  table[str(args_tuple[1])]["Serial"] = calculate_new_mean(serial_results[str(args_tuple)]["times"])
  for procs_threads in args["procs_threads"]:
    table[str(args_tuple[1])][str(procs_threads)]= calculate_new_mean(parallel_results[str(args_tuple)][procs_threads]["times"])
